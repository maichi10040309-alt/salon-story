...
assert.equal(migrated.gameVersion,'0.4');assert.equal(migrated.money,765432);assert.equal(migrated.day,22);assert.equal(migrated.customers.find(c=>c.id==='misaki').trust,88);assert.equal(migrated.customers.find(c=>c.id==='misaki').lastVisitDay,21);assert.equal(migrated.staff.level,5);assert.equal(migrated.staff.bond,0);assert.equal(migrated.wardrobe.owned.length>=3,true);assert.equal(migrated.stores.length,1);
console.log('Salon Story Ver.0.4 systems tests: OK');
